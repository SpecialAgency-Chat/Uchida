  ...
  const ons = async (token: string) => {
    try {
      const result = await fetch(
        ...
        }/joinrequest?timestamp=${new Date().getTime()}`,
        {
          method: "POST",
          headers: {
      ...
      if (resultJson.error) {
        ...
        setStatus("error");
        setError("Unknown error. View browser console for more information.");
      } else {
        setStatus("success");
      }
    } catch (e) {
    ...
    <main className="flex min-h-screen flex-col items-center p-24 gap-4 text-lg">
      {status === "loading" ? (
        ...
        <>
          ...
          <p className="mb-6">
            Join request succeed. Request will be reviewed by SAC staffs.
          </p>
        </>
      ) : status === "error" ? (
        <>
